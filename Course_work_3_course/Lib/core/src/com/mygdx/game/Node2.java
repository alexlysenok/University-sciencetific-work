...
private final int privDistance = 5;
    ...
    public Node2(float startX, float startY, int pId){
    	rnd=new Random();
    	initRandomValues();
    	stoped=false;
    	evading = false;
    ...
    public void initRandomValues()   // инициализация значений
	{
    	int  xDest;
        int  yDest;
        boolean xLeftDest=rnd.nextBoolean();
        boolean yTopDest=rnd.nextBoolean();
		
        
        	xLeftDest=rnd.nextBoolean();
        	yTopDest=rnd.nextBoolean();
  		  if(xLeftDest==true)
  	        {
  			xDest=rnd.nextInt(400);
  	        }
  		  else xDest=rnd.nextInt(400)+624;
  		 
  		  if(yTopDest==true)
	        {
  			yDest=rnd.nextInt(400);
	        }
		  else yDest=rnd.nextInt(400)+624;
	...
	{		
 		...
 		{	
 		 if(TestGame.nodesList.get(i).id!=id)       // проверка на сопадение айдишника
 		 {			 
 			 if (Math.abs((int)(TestGame.nodesList.get(i).myYy-myYy)) <=radius && Math.abs((int)(TestGame.nodesList.get(i).myXx-myXx)) <=radius)    // проверка на совпадение координат
 		     {				 
 				neighbours++;	 
 		     }			 
 		 } 		 				 
 		...
 		{	
 		 if(TestGame.nodesList.get(i).id!=id)       // проверка на сопадение айдишника
 		 {
 		     ...
 		     {				 
 				 ...
 				 if(Math.abs((int)(TestGame.nodesList.get(i).myYy-nextY)) >radius || Math.abs((int)(TestGame.nodesList.get(i).myXx-nextX)) >radius)
 				 {
 					 if(neighbours<=TestGame.density)
 					 {
 					 return true;
 					 }
 				 }
