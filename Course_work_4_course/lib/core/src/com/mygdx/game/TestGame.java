...
public class TestGame implements ApplicationListener {
	...
	public void create() {
		...
		flags.add(new Flag(500, 200, stage));
		flags.add(new Flag(700, 300, stage));
		flags.add(new Flag(400, 400, stage));
		...
		flags.add(new Flag(1200, 480, stage));
	...
	for(Flag flag: flags)	{
		...
		int j=5;
		flags.get(0).counter.time=10;
		
		for(int i=1; i<flags.size();i++){     // заполняем таймеры
			
			
			flags.get(i).counter.time=i*2+j;
	...
	public void render() {
		...
		if (startMoving) {
			...
			for(int i=0; i<flags.size(); i++){
				...
				font.draw(batch2, f.format((flags.get(i).counter.minusTime(Gdx.graphics.getDeltaTime()))), flags.get(i).counter.timerX-5f, flags.get(i).counter.timerY-15f);
			// font.draw(batch2, String.valueOf((int) (flags.get(i).counter.minusTime(Gdx.graphics.getDeltaTime()))), flags.get(i).counter.timerX-5f, flags.get(i).counter.timerY-15f);
				...
				flags.get(i).counter.currentTime=flags.get(i).counter.minusTime(Gdx.graphics.getDeltaTime());    
